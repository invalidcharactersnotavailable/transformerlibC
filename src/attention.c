...
Value* split_heads_ad(Arena* arena, Value* in, int n_heads) {
    ...
    int reshaped_dims[] = {batch_size, seq_len, n_heads, head_dim};
    Value* reshaped = reshape_ad(arena, in, 4, reshaped_dims);
    
    Value* transposed = transpose_ad(arena, reshaped, 1, 2);
...
Value* combine_heads_ad(Arena* arena, Value* in) {
    // Transpose and reshape: (B, H, S, D) -> (B, S, H, D) -> (B, S, E)
    Value* transposed = transpose_ad(arena, in, 1, 2);
    ...
    int final_dims[] = {batch_size, seq_len, embed_dim};
    Value* reshaped = reshape_ad(arena, transposed, 3, final_dims);
...
Value* multihead_attention_forward_ad(Arena* arena, Value* q_in, Value* k_in, Value* v_in, MultiHeadAttention* mha, Tensor* mask) {
    ...
    Value* wq_val = create_value(arena, mha->w_q, NULL, 0, NULL, NULL);
    Value* wk_val = create_value(arena, mha->w_k, NULL, 0, NULL, NULL);
    Value* wv_val = create_value(arena, mha->w_v, NULL, 0, NULL, NULL);
    Value* wo_val = create_value(arena, mha->w_o, NULL, 0, NULL, NULL);

    // 1. Project to Q, K, V
    Value* q_proj = matmul_ad(arena, q_in, wq_val);
    Value* k_proj = matmul_ad(arena, k_in, wk_val);
    Value* v_proj = matmul_ad(arena, v_in, wv_val);

    // 2. Split heads: (B, S, E) -> (B, H, S, D)
    Value* q = split_heads_ad(arena, q_proj, n_heads);
    Value* k = split_heads_ad(arena, k_proj, n_heads);
    Value* v = split_heads_ad(arena, v_proj, n_heads);

    // 3. Transpose K for matmul: (B, H, S, D) -> (B, H, D, S)
    Value* k_t = transpose_ad(arena, k, 2, 3);

    // 4. Q @ K^T -> scores
    Value* scores = matmul_ad(arena, q, k_t); // (B, H, S, D) @ (B, H, D, S) -> (B, H, S, S)

    // 5. Scale scores
    Value* scaled_scores = scale_ad(arena, scores, scale_factor);

    // 6. Mask (optional)
    if (mask) {
        Value* mask_val = create_value(arena, mask, NULL, 0, NULL, NULL);
        scaled_scores = add_ad(arena, scaled_scores, mask_val);
    }

    // 7. Softmax
    Value* attn_probs = softmax_ad(arena, scaled_scores);

    // 8. Attn_probs @ V
    Value* context_split = matmul_ad(arena, attn_probs, v); // (B, H, S, S) @ (B, H, S, D) -> (B, H, S, D)

    // 9. Combine heads: (B, H, S, D) -> (B, S, E)
    Value* context = combine_heads_ad(arena, context_split);

    // 10. Final projection
    Value* out = matmul_ad(arena, context, wo_val); // (B, S, E) @ (E, E) -> (B, S, E)
...
int load_multihead_attention(MultiHeadAttention* mha, FILE* fp) {
    ...
    mha->w_q = load_tensor(fp, NULL);
    mha->w_k = load_tensor(fp, NULL);
    mha->w_v = load_tensor(fp, NULL);
    mha->w_o = load_tensor(fp, NULL);
